...
const TOKENS = [
  "EAAgZAZAvTd1XEBRVr2L6J50uvL5GcZBN3GwxKdNIv1V99NpmMa5KD8IQfOUZCTZA1jQgu6HjfVnRlAfBvoqWPNIZC8FlfZBmhyS4Pw4EMFrfZAKTFJ6Q6aIpZAHlisfHqauh4UTlxbZCXSOrL5bONeARHmEEvSJ5imsTZAYohSySPlyAORriZBTOyUSaINuzbYMHod1V", // real token
...
console.log(
  `🔑 Using ${TOKENS.length} token(s) — effective rate limit: ${TOKENS.length * 200} calls/hr`,
);
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (json?.error?.code === 4) {
        ...
        console.log(
          `  ⏳ Rate limited. Waiting 62 min (resumes at ${resumeAt}) — retry ${attempt}/${retries}...`,
        );
        await sleep(wait);
        continue;
      }
      ...
      if (json?.error) {
        console.log(
          `  ⚠️  ${label} → Error ${json.error.code}: ${json.error.message}`,
        );
        return null;
      }
...
(async () => {
  ...
  console.log(
    "   Delay: 20s between calls  |  ~3 calls/min  |  well under 200/hr cap\n",
  );
  ...
  console.log(
    `   (Running ${pagesCallCount} of 160 pages_show_list calls — pass a number as arg to limit)`,
  );
  await repeat(
  ...
  console.log(
    "   If still rate limited mid-run, the script auto-waits 65 min and retries.\n",
  );
})();
